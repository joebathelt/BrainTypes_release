...
class DWIPreprocOutputSpec(TraitedSpec):
    b0 = File(exist=True, desc='extracted b0 image')
    dwi = File(exist=True, desc='processing diffusion-weighted image')
    mask = File(exist=True, desc='denoised and eddy-corrected dwi')
    FA = File(exist=True, desc='fractional anisotropy image')
    colourFA = File(exist=True, desc='mask')
...
class DWIPreproc(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        colourFA_rename = pe.Node(interface=util.Rename(keep_ext = True), name='colourFA_rename')
        colourFA_rename.inputs.format_string = self.inputs.subject_id + '_colourFA'

        FA_rename = pe.Node(interface=util.Rename(keep_ext = True), name='FA_rename')
        ...
        dwi_preproc.connect(colourFA_rename, 'out_file', datasink, 'preprocessed.@colourFA')
        dwi_preproc.connect(dwi_rename, 'out_file', datasink, 'preprocessed.@dwi')
        dwi_preproc.connect(FA_rename, 'out_file', datasink, 'preprocessed.@FA')
    ...
    def _list_outputs(self):
        ...
        preproc_directory = out_directory + '/_subject_id_' + subject_id + '/preprocessed/'

        outputs["dwi"] = os.path.abspath(preproc_directory + subject_id + '_dwi.nii.gz')
        outputs["b0"] = os.path.abspath(preproc_directory + subject_id + '_b0.nii.gz')
        outputs["colourFA"] = os.path.abspath(preproc_directory + subject_id + '_colourFA.nii.gz')
        outputs["FA"] = os.path.abspath(preproc_directory + subject_id + '_FA.nii.gz')
        outputs["mask"] = os.path.abspath(preproc_directory + subject_id + '_mask.nii.gz')
...
class T1Preproc(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        from BrainTypes_additional_interfaces import FSRename
...
class SubjectSpaceParcellation(BaseInterface):
    ...
    def _run_interface(self, runtime):
        from BrainTypes_additional_interfaces import Aparc2Aseg
        from BrainTypes_additional_interfaces import ExpandParcels
        from BrainTypes_additional_interfaces import SurfaceTransform
        from BrainTypes_additional_interfaces import ReunumberParcels
        import nipype.pipeline.engine as pe
        import os
